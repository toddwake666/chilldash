...
import { COLS, ROWS, WORLD, TRAFFIC, carPosition } from '@/src/game/world';
...
const Buildings = memo(function Buildings() {
  ...
  const names = ['CORNER STORE', 'SLICE OF HEAVEN', 'SUNSET STUDIOS', 'SUNNY SIDE CAFÉ', 'MAPLE APARTMENTS', 'CORNER NOODLES', 'YOUR GARAGE', 'BLOOM & GO', 'CITY LIBRARY', 'PALM HOUSE', 'THE ARCADE', 'OCEAN MARKET'];
  return <G>
    <Rect width={WORLD.width} height={WORLD.height} fill={c.grass} />
    <Rect x="1040" y="0" width="80" height="1360" fill={c.sky} />
    ...
    {ROWS.map(y => <G key={`h${y}`}><Rect y={y - 53} width={WORLD.width} height="106" fill={c.pavement} /><Rect y={y - 37} width={WORLD.width} height="74" fill={c.road} /><Line x1="0" y1={y} x2={WORLD.width} y2={y} stroke={c.butter} strokeWidth="2" strokeDasharray="18 20" /></G>)}
...
export function CityWorld({ player, heading, elapsed, route, target, minutes, weather, width, height, bike, gear }: { player: Point; heading: number; elapsed: number; route: Point[]; target: Point | null; minutes: number; weather: Weather; width: number; height: number; bike: string; gear: string }) {
  ...
  return <Svg testID="city-world" width={width} height={height} viewBox={`${cx - vw / 2} ${cy - vh / 2} ${vw} ${vh}`}>
    ...
    {night && <><Rect width={WORLD.width} height={WORLD.height} fill={c.night} opacity=".5" /><Circle cx={player.x} cy={player.y} r="55" fill={c.butter} opacity=".09" />{COLS.flatMap(x => ROWS.map(y => <G key={`light${x}${y}`}><Circle cx={x - 48} cy={y - 48} r="30" fill={c.brand} opacity=".09" /><Circle cx={x - 48} cy={y - 48} r="5" fill={c.butter} /></G>))}</>}
...
export function MiniMap({ player, target, route, size = 112, large = false }: { player: Point; target: Point | null; route: Point[]; size?: number; large?: boolean }) {
  const { colors: c } = useTheme();
  return <Svg testID={large ? 'phone-gps-map' : 'mini-gps-map'} width={size} height={large ? size * 1.18 : size} viewBox={large ? `35 20 1040 1290` : `${player.x - 330} ${player.y - 330} 660 660`}>
    <Rect width="1120" height="1360" fill={c.grass} />{COLS.map(x => <Rect key={x} x={x - 26} width="52" height="1360" fill={c.surface} />)}{ROWS.map(y => <Rect key={y} y={y - 26} width="1120" height="52" fill={c.surface} />)}
    {Array.from({ length: 12 }, (_, i) => <Rect key={i} x={COLS[i % 3] + 68} y={ROWS[Math.floor(i / 3)] + 65} width="144" height="148" rx="12" fill={i === 10 ? c.tree : i % 2 ? c.pavement : c.peach} />)}
